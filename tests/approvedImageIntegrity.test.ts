...
describe('the pipeline halts rather than discarding approved work', () => {
  ...
  it('repairs before it judges', () => {
    expect(orch.indexOf('const repaired = repairSceneVisuals('))
      .toBeLessThan(orch.indexOf('const hasExistingScenes ='));
  });

  it('halts, naming the scenes, when a project with approved images cannot be repaired', () => {
    expect(orch).toMatch(/This project has approved images, but \$\{broken\.length\} scene\(s\) cannot be rendered/);
    // The sentence spans two template literals, so assert the halves separately.
    expect(orch).toContain('The render stopped instead of ');
    expect(orch).toContain('rewriting the script and replacing your approved images');
  });

  it('still lets a project with no approved images script normally', () => {
    // The halt is gated on some scene actually carrying an approved image, so a
    // brand-new project is unaffected.
    expect(orch).toMatch(/if \(!hasExistingScenes && \(project\.scenes \|\| \[\]\)\.some\(\(s: any\) => s\.image_path\)\)/);
  });
});
