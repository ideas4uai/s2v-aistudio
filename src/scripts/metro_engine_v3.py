...
def render_scene_to_frames(cfg: EngineConfig, scene: SceneConfig,
                            prev_last_frame=None,
                            next_first_frame=None) -> List[np.ndarray]:
    """Render a single scene to a list of frames. Used by scene_animator_v3.py."""
    renderer = SceneRenderer(scene, cfg, prev_last_frame)
    return [renderer.render(fi) for fi in range(renderer.total_frames)]
